...
async function generateOptions(plateform, options) {
  const android = options.packages[plateform];
  android.orientation = android.orientation || {};
  android.sdkPath = await Editor.Profile.getConfig("program", "android_sdk") || "";
  android.ndkPath = await Editor.Profile.getConfig("program", "android_ndk") || "";
  if (!android.sdkPath || !android.ndkPath) {
    throw new Error(Editor.I18n.t("android.tips.android_sdk_error"));
  }
  return {
    packageName: android.packageName || "",
    orientation: {
      landscapeRight: android.orientation.landscapeRight ?? true,
      landscapeLeft: android.orientation.landscapeLeft ?? true,
      portrait: android.orientation.portrait ?? false,
      upsideDown: android.orientation.upsideDown ?? false
    },
    apiLevel: android.apiLevel,
    appABIs: android.appABIs || [],
    useDebugKeystore: android.useDebugKeystore,
    keystorePath: android.keystorePath || "",
    keystorePassword: android.keystorePassword || "",
    keystoreAlias: android.keystoreAlias || "",
    keystoreAliasPassword: android.keystoreAliasPassword || "",
    appBundle: android.appBundle ?? false,
    androidInstant: android.androidInstant ?? false,
    remoteUrl: android.remoteUrl || "",
    sdkPath: android.sdkPath || "",
    ndkPath: android.ndkPath || "",
    swappy: android.swappy || false,
    renderBackEnd: {
      vulkan: android.renderBackEnd?.vulkan ?? true,
      gles3: android.renderBackEnd?.gles3 ?? true,
      gles2: android.renderBackEnd?.gles2 ?? true
    },
    renderingScale: android.renderingScale,
    msaa: android.msaa
  };
}

let nativePackToolManager = null;
async function afterInit(plateform, options, result, cache) {
  const android = await generateOptions(plateform, options);
  options.packages[plateform] = android;
  ...
  cache.__addStaticsInfo(staticsInfo);
  const res = await checkAndroidAPILevels(plateform, android.apiLevel, options);
  if (res.error) {
  ...
  Editor.Metrics.trackEvent({
    ...
    label: "Vulkan",
    value: options.packages[plateform].renderBackEnd.vulkan ? 100 : 0
  });
  if (android.useDebugKeystore) {
    android.keystorePath = require$$1.join(Editor.App.path, "../tools/keystore/debug.keystore");
    android.keystoreAlias = "debug_keystore";
    android.keystorePassword = "123456";
    android.keystoreAliasPassword = "123456";
  }
  Object.assign(result.compileOptions.platformParams, android);
  if (renderBackEnd) {
    Object.keys(renderBackEnd).forEach((backend) => {
      result.compileOptions.cMakeConfig[`CC_USE_${backend.toUpperCase()}`] = renderBackEnd[backend];
    });
  }
  result.compileOptions.cMakeConfig.CC_ENABLE_SWAPPY = !!android.swappy;
  result.compileOptions.platform = "xr";
  lib.writeFileSync(result.paths.compileConfig, JSON.stringify(result.compileOptions));
  await getNativePackToolMg(result.compileOptions.enginePath, plateform);
  nativePackToolManager.init(result.compileOptions);
}
async function getNativePackToolMg(enginePath = "", plateform) {
  enginePath = enginePath || (await Editor.Message.request("engine", "query-engine-info")).typescript.path;
  const AndroidPackTool = require(require$$1.join(enginePath, "scripts/native-pack-tool/dist/platforms/android")).AndroidPackTool;
  const NativePackToolMg = require(require$$1.join(enginePath, "scripts/native-pack-tool/dist/base/manager")).nativePackToolMg;
  class XrPackTool extends AndroidPackTool {
    async copyPlatformTemplate() {
      await lib.copy(require$$1.join(this.paths.nativeTemplateDirInCocos, plateform, "build"), this.paths.nativePrjDir, { overwrite: false });
      await lib.copy(require$$1.join(this.paths.nativeTemplateDirInCocos, plateform, "template"), require$$1.join(this.paths.nativeTemplateDirInPrj, plateform), { overwrite: false });
    }
    resetPath() {
      this.paths.platformTemplateDirName = plateform;
  ...
  NativePackToolMg.register("xr", new XrPackTool());
  nativePackToolManager = NativePackToolMg;
  return nativePackToolManager;
}
async function afterBuild() {
}
async function beforeCompressSettings(plateform, options, result, cache) {
  result.settings.renderingScale = options.packages[plateform].renderingScale;
  result.settings.msaa = options.packages[plateform].msaa;
}
async function afterCompressSettings(plateform, options, result, cache) {
  const content = await lib.readJSON(result.paths.settings);
  content.screen.frameRate = 90;
  content.rendering = {
    ...content.rendering,
    renderingScale: options.packages[plateform].renderingScale,
    msaa: options.packages[plateform].msaa
  };
  lib.writeFileSync(result.paths.settings, JSON.stringify(content));
}
...
async function onAfterInit(options, result, cache) {
  ...
  params.cMakeConfig.XR_OEM_HUAWEI = "set(XR_OEM_PICO ON)";
  afterInit("xr-pico", options, result, cache);
}
async function onBeforeCompressSettings(options, result, cache) {
  beforeCompressSettings("xr-pico", options, result);
}
async function onAfterCompressSettings(options, result, cache) {
  afterCompressSettings("xr-pico", options, result);
}
